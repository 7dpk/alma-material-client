...
const headCells = [
  ...
  { id: "maths", numeric: true, disablePadding: false, label: "Maths" },
  {
    id: "percentage",
    numeric: true,
...
const handleDelete = (selected, rows, setRows, setMessage) => {
  selected.forEach((id) => {
    axios
      .delete(URL + id)
      .then(
  ...
  React.useEffect(() => {
    if (
      ...
      axios
        .get(URL)
        .then((res) => {
          localStorage.setItem("rows", JSON.stringify(res.data));
          setRows([...res.data]);
          console.log(rows);
        })
        .catch((e) => console.log(e));
